...
create_informative_labels <- function() {
  ...
  informative_labels_df <- tibble(
    lbs = c("HAoSMC", "HUVEC", "MCF10A", "PC3", "YAPC", "A375", "A549", "MCF7", "NPC", "Pericyte"),
    new_lbs = c(
      "HAoSMC", "HUVEC", "MCF10A  \n(fibrocystic)", "PC3    \n(prostate)",
      "YAPC    \n(pancreas)", "A375\n(skin)", "A549\n(lung)", "MCF7\n(breast)", "NPC  \n(neural)", "Pericyte"
    )
    ...
    mutate(
      color = ifelse(lbs == vascular_char_vec[1], colors[5], # check huvec
        ifelse(lbs == vascular_char_vec[2], colors[6], # check haosmc
          ifelse(lbs %in% other_char_vec, colors[3], colors[4])
        )
      ), # check other vs cancer
...
plot_pretty_dendrogram <- function(args,
  ...
  # load("debug/debug_dat/debug-gcp-perti-dendro.RData")
  # stop()
  data <- args$lst
  ...
  informative_labels_df_ordered <- tibble(lbs = labels(hclust_dend)) %>%
    mutate(dend_order = 1:nrow(.)) %>%
    left_join(informative_labels_df, by = "lbs") %>%
    left_join(create_ca_df(ca = cell_id_clusters_char_vec), by = c("lbs" = "cell_id"))

  if (rotate_dendrogram) {
    rotated_summary <- informative_labels_df_ordered %>%
      group_by(cluster) %>%
      summarize(
        cluster_name = str_c(sort(unique(lbs)),
          collapse = ","
        ),
        .groups = "keep"
      ) %>%
      ungroup() %>%
        arrange(cluster) %>%
        mutate(cluster_type = map_chr(cluster_name, .f = function(cn) {
          split_cn <- str_split(cn, pattern = ",", simplify = T)
          if (length(intersect(split_cn, vascular_char_vec)) > 0) {
            res <- "vascular"
          } else {
            res <- "non-vascular"
          }
    ...
    informative_labels_df_ordered_rotated <- left_join(informative_labels_df_ordered, rotated_summary, by = "cluster") %>%
      mutate(cluster_type = factor(cluster_type, levels = c("non-vascular", "vascular"))) %>%
      arrange(cluster_type, desc(grp_l)) %>% # sort by the cluster first, then by cell identity (e.g. vascular vs non-vascular)
      mutate(dend_order = 1:n())
    
    informative_labels_df_ordered_rotated
  } else {
    informative_labels_df_ordered_rotated <- informative_labels_df_ordered
    informative_labels_df_ordered_rotated
  }

  # this is the character vector used to sort the tree!!
  labels_char_vec <- informative_labels_df_ordered_rotated$new_lbs
  labels_char_vec
  ...
  to_plot <- function() {
    ...
    if (rotate_dendrogram) {
      ...
      rect.dendrogram(
        tree = cell_dend_color_leave_edges %>% rotate(labels_char_vec), # %>% rotate(labels_char_vec),
        ...
        xpd = NA
      ) # take the bottom border and increse slightly
    } else {
      rect.dendrogram(
        tree = cell_dend_color_leave_edges, # %>% rotate(labels_char_vec),
        k = max_n_clusts, # draw boxes around clusters # lower_rect = par("usr")[2L]*-0.25, # draw from right to left
        border = border_color_set, horiz = T, # horizontal plot
        lwd = rect_lwd_set, lty = rect_lty_set,
        lower_rect = dend_height * -0.1718011 - 0.08,
    ...
    if (plot_color_bar) {
      if (rotate_dendrogram) {
        ...
        new_color_order <- tibble(
          new_lbs = labels(cell_dend_color_leave_edges),
          order = 1:length(labels(cell_dend_color_leave_edges))
        )

        color_bars <- suppressMessages(left_join(
          informative_labels_df_ordered_rotated %>%
            dplyr::select(new_lbs, cell_individual_color, vasc_grp_color),
          new_color_order
        ) %>%
          arrange(order) %>%
        ...
        colored_bars(
          dend = cell_dend_color_leave_edges %>% rotate(labels_char_vec),
          horiz = T,
          colors = color_bars,
          sort_by_labels_order = FALSE, add = T, rowLabels = ""
        )
      } else {
        color_bars <- informative_labels_df_ordered_rotated %>%
          dplyr::select(vasc_grp_color, color)
        
        colored_bars(
          dend = cell_dend_color_leave_edges, horiz = T,
          colors = color_bars,
          sort_by_labels_order = FALSE, add = T, rowLabels = ""
        )
      }
    ...
    # stop()
    non_vasc_color <- unique(informative_labels_df_ordered_rotated %>%
      filter(grp != "Vascular cells") %>%
      .$vasc_grp_color)
    vasc_color <- unique(informative_labels_df_ordered_rotated %>%
      filter(grp == "Vascular cells") %>%
      .$vasc_grp_color)
    ...
    if (plot_legend) {
      leg_temp <- bind_rows(
        informative_labels_df_ordered_rotated %>% distinct(grp_l, color, vasc_grp_color) %>% rename(grp = grp_l),
        informative_labels_df_ordered_rotated %>% distinct(grp_l2, color, vasc_grp_color) %>% rename(grp = grp_l2),
        informative_labels_df_ordered_rotated %>% distinct(grp, color, vasc_grp_color)
      ) %>%
        ...
        arrange(grp)
      leg_ <- bind_rows(
        leg_temp %>% slice(1:4) %>% dplyr::select(grp, color),
      ...
      legend(
        x = xpos, y = ypos, title = "Legend", text.font = 3,
        legend = leg_final,
        col = col_final,
  ...
  plot_eps <- function() {
    setEPS()
    postscript(
      bg = "white", file = dendro_output_fn,
    ...
    # par(mar = c(2,2,2,2),
    par(
      mar = c(7, 14, 7, 14), # c(bottom, left, top, right)
      cex = 2.2, # adjust plotting text size (not title or legend)
      xpd = TRUE
    ) # allow plotting outside window
