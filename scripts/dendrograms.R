...
#' @note 
#' @param data pert or moa specific lst-dataset, from *cluster3.R*
#' @param base_output_dir dataset specific outputdir, eg. ~/Downloads/p100
#' @param INSET_MARGIN percent of plotsize in x direction to adjust legend, usually negative
plot_pretty_dendrogram <- function(data, 
                                   base_output_dir = "~/Downloads", 
                                   # LOWER_RECT_BORDER = -0.25,
                                   INSET_MARGIN = -0.06, 
                                   rotate_dendrogram = FALSE) {
  # data <- p100_pert_data_dmso
  # base_output_dir <- p100_base_output_dir
  
  dataset <- data$which_dat[[1]]
  output_dir <- file.path(base_output_dir, "prettydendros")
  dir.create(output_dir, recursive = T, showWarnings = F)
  
  if ("pert_iname" %in% colnames(data) & "drug_class" %in% colnames(data)){
    org <- "pert_iname"
    name <- data$pert_iname[[1]]
    dendro_output_fn <- file.path(output_dir, qq("@{name}_@{dataset}_pdendro.eps")) # pdf
    
    message(qq("Writing to: \n@{dendro_output_fn}"))
    plot_title <-  str_c("Hierarchical clustering on connectivity matrix\n", 
                         str_c(unique(data$pert_iname[[1]]), toupper(unique(data$which_dat[[1]])), sep = ' || ')); plot_title
    ...
    message(qq("Writing to: \n@{dendro_output_fn}"))
    plot_title <-  str_c("Hierarchical clustering on connectivity matrix\n", 
                         str_c(unique(data$drug_class[[1]]), toupper(unique(data$which_dat[[1]])), sep = ' || ')); plot_title
    ...
    message(qq("Writing to: \n@{dendro_output_fn}"))
    plot_title <-  str_c("Hierarchical clustering on connectivity matrix\n", 
                         str_c(unique(data$org[[1]]), toupper(unique(data$which_dat[[1]])), sep = ' || ')); plot_title
    
  }
  
  cell_id_clusters_char_vec <- data$cut_trees[[1]]; cell_id_clusters_char_vec
  pvclust_obj <- data$pvclust[[1]]; # pvclust_obj
  co_clust <- data$co_clust[[1]]
  max_n_clusts <- max(cell_id_clusters_char_vec)
  
  dend_height <- max(pvclust_obj$hclust$height) # height of the first split!, total height of dendro
  # print(dend_height)
  hclust_dend <- as.dendrogram(pvclust_obj$hclust)
  ...
  if ( !any(vasc_dend_pos %in% last_2_positions)  ) { #& co_clust
    ...
    vasc_top_labels_df <- tibble(lbs = c(names_vasc_dend_pos,names_last_2_position), 
      ...
      left_join(vasc_top_labels_df %>% select(-dend_order),by="lbs") %>% # give their lbs their properties back
  ...
  if (rotate_dendrogram){
    ...
    color_bars <- left_join(vasc_top_labels_df_rotated_order %>% select(new_lbs, color, vasc_grp_color), new_color_order ) %>%
      arrange(order) %>% 
      select(vasc_grp_color, color)
  ...
  leg_temp <- bind_rows(informative_labels_df_ordered %>% distinct(grp_l, color, vasc_grp_color) %>% rename(grp = grp_l),
    ...
    arrange(grp)
  leg_ <- bind_rows(leg_temp %>% slice(1:4) %>% select(grp, color), 
...
# i <- which(p100_res$moa$drug_class %in% moa_interest_char)
# p100_moa_data_ki <- p100_res$moa[i,] # choose row
# 
# i <- which(gcp_res$moa$drug_class %in% moa_interest_char)
