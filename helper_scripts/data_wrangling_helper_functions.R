...
#' @param path any path
my.file.path <- function(path) { return(chartr("\\","/", path))}
...
load_data <- function(data, drug_classes_fn = "~/Drug Glossary_edited.xlsx", dataset="p100",
  ...
  det_norm_group_vect <- data[2,] %>%
    ...
    distinct(); dim(det_norm_group_vect)
  ...
  phosphosite_long_df <- data %>%
    ...
    distinct(pr_gene_symbol, pid);  dim(phosphosite_long_df)
  ...
  datb <- data %>% 
    ...
    left_join(drugs_moa_df, by = "pert_iname"); dim(datb)
  ...
  cell_lines_identities <- datbb %>% 
    ...
    select(-cell_cat); cell_lines_identities
  ...
  if (filter_by_vasc_perts){
    ...
    tested_drugs_in_smcs <- tested_drugs_in_cv_cells %>% filter(cell_id == "HAoSMC") %>% .$pert_iname %>% unique()
    tested_drugs_in_ecs <- tested_drugs_in_cv_cells %>% filter(cell_id == "HUVEC") %>% .$pert_iname %>% unique()
    ...
    no_replicates_dat <- datbb %>% 
      ...
      select(cell_id, pert_iname, drug_class, pr_gene_symbol, pex_m, drug_cat, cell_cat, phosphosite, is_cancer); no_replicates_dat
    ...
    perturbation_feature_set <- datbb %>% distinct(pert_iname) %>% .$pert_iname
    feature_set <- datbb %>% distinct(pr_gene_symbol) %>% .$pr_gene_symbol
...
load_wrapper <- function(datasets_directory = NULL, references_directory = NULL, 
  ...
  if (load_from_file){
    if (file.exists(output_fn)) return(read_rds(output_fn))
    else message("Rds doesn't exist. Creating analysis-ready data object...")
  } 
...
get_replicates <- function(dat_obj, grouping_var = "pert_iname"){
  ...
  features <- dat_obj$feature_set; features
  drug_classes <- dat_obj$drug_mapping; drug_classes
  ...
  if (tolower(grouping_var) == "pert_iname"){
    ...
    mat_reps_temp_b <- mat_reps_temp_a %>%    
      ...
      mutate(data_by_cell = pmap(.l = list(data_by_tx_group, pert_iname, drug_class), function(x,y,z){
        ...
        pc <- x %>% 
          mutate(pert_iname = y, # give drug class and pert_iname a column outside of the nested datasets
                 drug_class = z) %>%
  ...
  mat_reps <- mat_reps_temp_b %>%
    ...
    mutate(df = map(data_by_cell, function(cell_dat_df){
      ...
      cell_dat <- cell_dat_df$data_per_analyte; cell_dat
      nrows_df <- cell_dat_df %>% 
        mutate(n_rows = map_int(data_per_analyte, function(x) nrow(x))); nrows_df
      ...
      well_by_analyte_df_plus_annots <- map_df(cell_dat, function(analyte_df){
        ...
        wide_analyte_df <- analyte_df %>% 
         group_by(well, pr_gene_symbol) %>%  # if there are dup wells, compress by median
          ...
          spread(pr_gene_symbol, pex); wide_analyte_df
  ...
  final_mat_reps <- mat_reps %>%
    ...
    mutate(dataframe = map(df, .f = convert_to_dataframe, keep_cols_vec = features),
...
match_function <- function(cur_mat, master_df = NULL, 
  ...
  ordered_names_df <- tibble(rnames = rownames(cur_mat),
...
make_numeric_mat <- function(long_df){
  ...
  x <- long_df %>% 
    spread(group_b, conn) %>% 
    as.data.frame() %>% select(-group_a) %>% as.matrix()
...
flattenMatrix <- function(m) {
  ut <- upper.tri(m)
  lt <- lower.tri(m)
  bind_rows(data.frame(i = rownames(m)[row(m)[ut]],
                       j = rownames(m)[col(m)[ut]],
                       cor=t(m)[ut],
                       stringsAsFactors = F), 
            data.frame(i = rownames(m)[row(m)[lt]],
...
make_avg_df <- function(df1, df2, vars_to_join=c("well", "cell_id","pert_iname","drug_class")){
  ...
  # 
  join1 <- temp1 %>% mutate(well = temp1$idx) %>% select(-twell, -idx)
  join2 <- temp2 %>% mutate(well = temp2$idx) %>% select(-twell, -idx)
